...
VALID_PACK = {
    "flux_final_prompt": "A clearly adult fictional character in a coherent portrait scene.",
    "z_image_positive_prompt": "clearly adult fictional character, coherent portrait scene",
    "z_image_negative_prompt": "underage, minor, childlike, youthful appearance",
...
def test_exporter_requires_all_and_only_contract_fields() -> None:
    ...
    with pytest.raises(ValueError, match="missing fields"):
        validate_prompt_pack({"flux_final_prompt": "only one field"})
...
def test_exporter_writes_only_to_explicit_absolute_path(tmp_path: Path) -> None:
    ...
    assert "## Flux Final Prompt" in text
    assert "## Z-Image Final Negative Prompt" in text
