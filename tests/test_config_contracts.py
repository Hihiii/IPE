...
def test_always_nsfw_baseline_resolver_routes_only_eligible_humans() -> None:
    ...
    assert "check" in wardrobe["unspecified_wardrobe"]
    assert "nude trigger" in wardrobe["unspecified_wardrobe"]["check"]
    assert "auto_outfit_selected" in wardrobe
...
def test_visible_exposure_contract_requires_readable_feminine_evidence() -> None:
    ...
    assert exposure["resolution"]["auto_outfit"]["exposure_requirement"] == "partial_nudity_required"
    assert exposure["resolution"]["auto_outfit"]["garment_transformation_action"] == "revealing_outfit_worn"
...
def test_comfyui_output_contract_is_complete() -> None:
    ...
    assert "flux" in overrides["disabled_delivery_targets"]
...
def test_regression_cases_match_output_contract() -> None:
    ...
    assert {"plain_human_always_nsfw", "clothed_human_transformation", "unspecified_wardrobe_auto_outfit", "explicit_nude_baseline", "auto_outfit_variant_trigger", "human_edit_auto_adultization", "nonhuman_no_nsfw_injection"}.issubset(ids)
