...
def validate_visible_exposure_contract(value: Any, prompt_pack: dict[str, str]) -> None:
    ...
    positive_prompts = {"z_image_positive_prompt": z_positive}
    if subject == "female_feminine" and isinstance(target, str):
        ...
        if wardrobe_state == "unspecified_wardrobe" and any("nude" not in prompt and "bare" not in prompt for prompt in positive_prompts.values()):
            failures.append(_failure("exposure_prompt_nudity", "Unspecified wardrobe must state the full nude adult presentation in the Z-Image positive prompt."))
        if evidence_mode == "sheer_visible_anatomy" and any("sheer" not in prompt and "translucent" not in prompt for prompt in positive_prompts.values()):
            failures.append(_failure("exposure_sheer_evidence", "Sheer evidence must name sheer or translucent material in the Z-Image positive prompt.", target=target))
...
def validate_exposure_action_plan(
    ...
    positive_prompts = {"z_image_positive_prompt": prompt_pack.get("z_image_positive_prompt", "").casefold()}
    if action_definition is not None:
        for field, prompt in positive_prompts.items():
