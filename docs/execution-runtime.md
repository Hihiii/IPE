...
4. Validate the record. Success prints the four prompt-pack fields; failure
   writes a machine-readable trace in the system temporary directory unless a
   path is explicitly supplied.

```powershell
python scripts/compile_execution_packet.py --input request.json --output C:\temp\packet.json
